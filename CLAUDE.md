...
The internal website for the **NC State Robotics Club** — Next.js 16 + TypeScript + Tailwind v4, JSON-backed, static-first. The repo is a working **Phase 1** build of a three-phase plan. (The group has no physical lab; the displayed name is "NC State Robotics Club" / short "Robotics Club", set in `data/lab.json`. Generic "lab" wording inside these planning docs is incidental.)

## Read first (in order)

When the task touches structure, content, or design decisions, read these before editing. They live in the `docs/` folder (this file and `README.md` stay at the repo root).

1. **`docs/pages.md`** — per-page spec, canonical subgroup names, website-vs-Discord responsibility split. Most authoritative doc.
2. **`docs/phase.md`** — three-phase rollout. Tells you whether a feature belongs to Phase 1 (now), Phase 2 (later), or Phase 3 (much later).
3. **`docs/webflow.html`** — visual sitemap. Open in a browser to see how pages connect; has a light/dark toggle.
4. **`README.md`** — how to run, where to drop real Google Form / Calendar URLs, repo layout.
5. **`docs/admin.md`** — only when the task touches the gated `/admin/*` area: the Basic Auth gate (`proxy.ts`), env vars, Vercel deploy, the group mailer, and the meeting scheduler.
6. **`docs/supabase.md`** — only when the task touches the Supabase backend (meetings): project setup, the `meetings` table schema + RLS, env vars, `lib/supabase.ts` clients, and the seed script.
7. **`docs/resources-submission.md`** — only when the task touches the resource submission flow: the Google Form, the routing Apps Script, the `npm run import:resources` script, and the "approve = commit" publishing model.
...
- **Admin area (out of phase).** Two gated tools live under `/admin/*`, behind a shared-password Basic Auth gate (`proxy.ts`, `ADMIN_USER`/`ADMIN_PASS`), reachable via a discreet footer link → `/admin` landing page (not in nav): the **group mailer** (`/admin/email`, built on `lib/mailto.ts` + `components/ComposeLinks.tsx`) and the **meeting scheduler** (`/admin/schedule`, below). This is a deliberate narrow exception to the "no auth in Phase 1" rule — full per-user auth remains unbuilt. See `docs/admin.md`.

- **Meeting scheduler + Supabase (Phase 3 pulled forward).** `/admin/schedule` lets the faculty lead add/edit/delete meetings; they persist in a **Supabase** `meetings` table — now the source of truth, replacing `data/meetings.json`. The public `/schedule` page (next-meeting hero + upcoming + past; the Google Calendar embed was dropped) and home next-meeting card read from Supabase (meeting accessors in `lib/data.ts` are async; those pages are `force-dynamic`). Writes use a secret-key client (`lib/supabase.ts`) from gated server actions. A meeting's track is `MeetingTrack` = `general` (club-wide, default) | `ai` | `mechatronics` — **"general" is meeting-only, not a real group** (keeps `GroupSlug`/membership/mailer untouched). Join info (Zoom link / meeting ID / passcode) is optional and public; meetings can repeat weekly/biweekly. See `docs/supabase.md` and `docs/admin.md`.

- **Resource submission (pulled forward from Phase 2).** A submit flow now exists: a `/resources` CTA links out to a Google Form; an admin reviews and publishes by running `npm run import:resources <csv>` (exported from the responses Sheet) and committing the new `data/resources.json` entries. No backend — "approve" = a git commit. See `docs/resources-submission.md`.

- **Resources hidden from nav (deployment).** The `/resources` page, its data, and the submission flow are intact, but the **nav links are commented out** in `components/Header.tsx` and `components/Footer.tsx` — the club isn't publishing Resources yet. The route still resolves if visited directly. Re-enable by uncommenting both entries. Don't delete the resources code/data.

Before building a new feature, check `docs/phase.md` to confirm which phase it belongs to. Do not silently pull Phase 2/3 work into the codebase — flag it to the user and ask.

## Hard rules

1. **Website = source of truth. Discord = daily activity.** Do not add chat, polls, real-time announcements, role pickers, or anything that duplicates Discord. (See `docs/pages.md` "Core principle".)
2. **Canonical names live in `docs/pages.md`.** Match those strings exactly when editing `data/subgroups.json`, member subgroup affiliations, etc. `docs/webflow.html` uses display abbreviations to fit boxes — don't copy from it as source of truth.
...
6. **Keep the planning docs in sync.** When code or content changes, update the matching doc in the *same* change so the docs don't drift from reality. Specifically:
   - Renaming or adding a subgroup → update `docs/pages.md` (canonical names list), `data/subgroups.json`, any affected `data/members.json` entries, and consider whether `docs/webflow.html` needs a redraw.
   - Adding, removing, or moving a page → update `docs/pages.md` (per-page spec), `docs/webflow.html` (sitemap), and the nav list in `components/Header.tsx`.
   - Pulling a feature forward into an earlier phase (or pushing one back) → update `docs/phase.md` so the phase boundaries stay honest. Mention the move in the PR/commit.
   - Changing the website-vs-Discord split → update the "Core principle" + responsibility table in `docs/pages.md`.
   - Changing the meeting cadence, mission, or institution → update `data/lab.json` (the rendered surface) **and** `docs/pages.md` if the cadence is restated there.
